...
const SubmissionsTab: React.FC = () => {
  ...
  //  console.log(slug)
  const { submissions = [], loading, error,fetchMySubmissionsBySlug } = useSubmission();
  const navigate = useNavigate();
  const { hasRefreshed, isLoggedIn} = useAuth()
  // console.log(submissions)
 useEffect(() => {
    if (slug && hasRefreshed && isLoggedIn) fetchMySubmissionsBySlug(slug);
  }, [fetchMySubmissionsBySlug, hasRefreshed, isLoggedIn, slug]);
  ...
  if (error) {
    return (
      <div className="p-4">
        <CustomMessage type="error" message={error} onClose={() => {}} />
      </div>
    );
  }
  ...
  const handleRowClick = (problemSlug: string, submissionId: string) => {
    navigate(`/problems/${problemSlug}/submissions/${submissionId}`);
  };
        ...
        <tbody>
          {submissions.map((sub) => (
            <tr
              key={sub._id}
              onClick={() =>
                handleRowClick(slug || "unknown-problem", sub._id)
              }
            ...
            >
              ...
              <td className="px-4 py-2">{sub.executionTime ? `${sub.executionTime} ms` : "N/A"}</td>
              <td className="px-4 py-2">
